  ...
  def enum_notepadpp_windows(hwnd, lparam):
    ...
    if u'Notepad++' in wnd_class_name_buf.value:
      ...
      if process_id.value == in_params.current_pid:
        ...
        # filter out all `-z` parameters to leave only Notepad++ built in parameters except several specific
...
def process_extra_command_line():
  ...
  remove_file_list_after_open = False
  ...
  try:
    ...
    for arg in cmdline_list:
      ...
      if next_arg_is_file_list_path:
        ...
        open_path_len_limit = max(int(arg), 255) # 255 is built in minimum
        next_arg_is_open_path_limit = False
        print('--open_short_path_if_gt_limit ' + str(open_path_len_limit))
      elif next_arg_is_child_cmdline_len_limit:
        # 1 is built in minimum, means: use one file to open per each child Notepad++ command line
        # 32767 is built in maximum for Win32 CreateProcess
        ...
        elif arg == '--remove_file_list_after_open':
          remove_file_list_after_open = True
    ...
    if open_from_file_list_path:
      ...
      if do_append:
        ...
        # enumerate Notepad++ instances as stand alone windows
        enum_notepadpp_windows(enum_npp_windows)

        if do_restore_if_open_inplace:
          if enum_npp_windows.notepadpp_hwnd and not len(enum_npp_windows.not_multiinst_pid_list): # open in place
      ...
      if from_utf8:
        ...
        file_content_decoded = file_content.decode('mbcs', errors = 'ignore')
        ...
        if file_path:
          ...
          if do_open_as_is:
            ...
            if kernel32.GetShortPathNameW(file_path_unc, file_path_short_buf, file_path_short_buf_size):
              ...
              if do_open_inplace:
                if os.path.isfile(str(file_path_short_buf.value)):
      ...
      if len(append_cmdline_file_list):
        ...
        while len(append_cmdline_file_list):
          ...
          if not do_append_by_child_instance and len(enum_npp_windows.not_multiinst_wnd_list):
            ...
            #   This implementation is left as an alternative and the last method.
            #
    ...
    if remove_file_list_after_open and open_from_file_list_path:
      os.remove(open_from_file_list_path)
...
# `allow_not_selected` has meaning if `use_selection=True` and text is not selected
...
def open_new_instance_from_current_tab_text_as_file_path_list(use_selection = False, allow_not_selected = True):
  ...
  with tempfile.NamedTemporaryFile(mode='w+t', newline=lr, encoding='utf-8', delete=False) as tmp_file:
    ...
    subprocess.Popen(
      [
        sys.argv[0], '-nosession', '-multiInst',
        '-z', '--from_utf8', '-z', '--remove_file_list_after_open',
        '-z', '--open_from_file_list', '-z', tmp_file.name.replace('\\','\\\\')
      ],
